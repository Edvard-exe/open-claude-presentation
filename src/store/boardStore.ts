...
export const useBoardStore = create<BoardState>((set) => ({
  ...
  tiles: [
    ...
    // ════════════════════════════════════════════════════════════
    {
      ...
      storySteps: [
        {
          label: 'Estimate', title: 'Token Estimation — chars / 4', source: 'src/services/compact/autoCompact.ts',
          description: 'Claude Code never calls the tokenizer API — too slow. Instead it estimates: chars ÷ 4 (default bytesPerToken). Compaction fires at 70% of the context window.',
          ...
          codeLang: 'typescript',
          spark: 'chars ÷ 4 by default. A rough estimate that\'s fast enough to run every turn without slowing the loop.',
        },
        {
